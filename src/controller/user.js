         ...
         if(login==undefined){ //user not found
            ...
            let sort=["date","fname"];
                    ...
                    if(!isNaN(req.body.page)){
                     ...
                     console.log("query started");
                     var countQuery = function(callback){
                        User.find({email: "/.*"+req.params.query+".*/i"}, function(err, doc){
                              if(err){ callback(err, null) }
                              else{
                                  callback(null, doc.length);
                               }
                   ...
                   var retrieveQuery = function(callback){
                       console.log((pageNumber-1)*12);
                       User.find({email: "/.*"+req.params.query+".*/i"}).skip((pageNumber-1)*12).sort({sortby: -1}).limit(12).exec(function(err, doc){
                        if(err){ callback(err, null) }
                        else{
                            callback(null, doc);
                         }
